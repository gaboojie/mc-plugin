...
public class DifficultyListener implements Listener {
    ...
    public boolean shouldUpdateDisplayName(LivingEntity entity) {
        return switch (entity.getType()) {
            case PLAYER, ENDER_DRAGON, END_CRYSTAL, WITHER, WARDEN, VILLAGER, ELDER_GUARDIAN, IRON_GOLEM, ZOMBIE_VILLAGER -> false;
            default -> true;
        };
    }

    public void updateDisplayName(LivingEntity entity, double health) {
        // Ensure that we should update the display name
        if (!shouldUpdateDisplayName(entity)) {
            return;
        }
